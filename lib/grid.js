...
exports.broadcast = function (msg, exclude) {
    for (var id in this._sockets) {
        if (id === exclude) { continue; }
        try {
            this._sockets[id].send(msg, { binary: true });
        } catch (e) {
            this.emit('broadcastError', e);
        }
    }
};
...
exports.connectionHandler = function (id, socket) {
    ...
    this.broadcast(Message.Connected(id).toBuffer(), id);
};
...
exports.disconnectHandler = function (id) {
    ...
    this.broadcast(Message.Disconnected(id).toBuffer(), id);
};
